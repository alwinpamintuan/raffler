...
function Sidebar() {
  ...
  const introduction = `Welcome to Raffler!

Click here to place entries.
...
Entry 3
  ...
  return (
    <div id="sidebar">
      <div id="input-area">
        <h3>
          Entries <span id="count">({entries.length})</span>
        </h3>
        <textarea
          id="entry-field"
          name="entries"
        ...
        <div>
          <input
            ...
            defaultChecked
          ></input>
          <label htmlFor="remove-winners">
            {" "}
            Remove entry of previous winner
          </label>
        </div>
      </div>
      ...
      <div id="winners">
        ...
        <ul id="recent-winners">
          {winners
            ...
            .reverse()
            .map((winner, idx) => {
              return <li key={idx}>{winner.name}</li>;
            })}
        </ul>
      </div>
    </div>
